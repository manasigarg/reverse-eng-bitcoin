...
def privkey_to_pubkey(privkey):
    ...
    logger.debug("privkey: {}".format(privkey))
    if privkey >= N:
...
def pubkey_to_address(pubkey, magicbyte=0):
    if isinstance(pubkey, (list, tuple)):
        logger.info("Pubkey in tuple form")
        pubkey = encode_pubkey(pubkey, 'bin')
    if len(pubkey) in [66, 130]:
        logger.info("Pubkey in range 66 to 130")
        unhexified_pubkey = binascii.unhexlify(pubkey)
        logger.info("bytes: {}".format(unhexified_pubkey))
        hashed160 = bin_hash160(unhexified_pubkey)
        logger.info("hashed bytes: {}".format(hashed160))
        base58_check = bin_to_b58check(hashed160)
        logger.info("base58 encoded: {}".format(base58_check))
        return bin_to_b58check(bin_hash160(binascii.unhexlify(pubkey)), magicbyte)
    logger.info("bin_to_b58check(bin_hash160(pubkey), magicbyte): ".format(bin_to_b58check(bin_hash160(pubkey), magicbyte)))
